...
from src.utils.pose_utils import quat2mat_torch, quat_trans_to_pose_m
        ...
        for i in range(pred_quats.shape[0]):
            ...
            if is_allo:
                ...
                cur_ego_mat = allocentric_to_egocentric(
                    quat_trans_to_pose_m(pred_quats[i], translation[i].detach().cpu().numpy()),
                    src_type="mat",
                    dst_type="mat",
                )[:3, :3]
            else:
                cur_ego_mat = quat_trans_to_pose_m(pred_quats[i], translation[i].detach().cpu().numpy())
