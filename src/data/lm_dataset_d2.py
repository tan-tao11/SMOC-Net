...
from src.utils.utils import dprint, iprint, lazy_property
    ...
    def __call__(self):  # LM_Dataset
        ...
        for ann_file, scene_root, xyz_root in zip(tqdm(self.ann_files), self.image_prefixes, self.xyz_prefixes):
            ...
            for im_id in tqdm(indices):
                ...
                rgb_syn_path = osp.join(scene_root, "rgb_syn_{}/{:06d}.png".format(self.objs[0], int_im_id))
                mask_syn_path = osp.join(scene_root, "rgb_syn_{}/{:06d}_mask.png".format(self.objs[0], int_im_id))
                ...
                record = {
                    ...
                    "rgb_syn_file": osp.relpath(rgb_syn_path, PROJ_ROOT),
                    "mask_syn_file": osp.relpath(mask_syn_path, PROJ_ROOT),
                    "height": self.height,
                    "width": self.width,
        ...
        if name not in SPLITS_LM:
            SPLITS_LM[name] = dict(
                ...
                ann_files=[
                    osp.join(
                        DATASETS_ROOT,
                        "BOP_DATASETS/lmo/image_set/lmo_no_bop_test_{}.txt".format(obj),
                    )
                ],
                # NOTE: scene root
                image_prefixes=[osp.join(DATASETS_ROOT, "BOP_DATASETS/lmo/test/{:06d}").format(2)],
...
for obj in ref.lm_full.objects:
    for split in ["train", "test"]:
        ...
        im_ids = []
        if not os.path.exists(cur_ann_file):
            continue
