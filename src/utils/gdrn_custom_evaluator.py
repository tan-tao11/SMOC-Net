...
import ref
from src.utils.common import all_gather, is_main_process, synchronize
from src.utils.pose_utils import get_closest_rot
from utils.gdrn_visualizer import MyVisualizer, _RED, _GREEN, _BLUE, _GREY
from src.utils.data_utils import crop_resize_by_warp_affine
from src.utils.pysixd import inout, misc
from src.utils.pysixd.pose_error import add, adi, arp_2d, re, te
from src.utils.mask_utils import binary_mask_to_rle
from src.utils.utils import dprint
from src.utils.vis_utils.image import grid_show, vis_image_bboxes_cv2
        ...
        for i, (_input, output) in enumerate(zip(inputs, outputs)):
            ...
            for inst_i in range(len(_input["roi_img"])):
                ...
                if False:  # visualize pose
                    ...
                    # )
                    visualizer_full = MyVisualizer(im_ori[:, :, ::-1], self._metadata)
                    full_im_vis = visualizer_full.draw_bbox3d_and_center(
                        kpt_2d_full,
                        top_color=_GREEN,
                        bottom_color=_GREY,
                        linewidth=linewidth,
                    ...
                    save_path = osp.join(
                        cfg.output_dir,
                        "vis",
                        "{}_{}_{:06d}_gt_est_full.png".format(cls_name, scene_id, im_id),
                    )
                    mmcv.mkdir_or_exist(osp.dirname(save_path))
                    full_im_vis.save(save_path)
                    print("full_im_vis saved to:", save_path)
