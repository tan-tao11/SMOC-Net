...
from src.utils.torch_utils.solver.lr_scheduler import flat_and_anneal_lr_scheduler
from src.utils.torch_utils.solver.optimize import _get_optimizer
...
def build_lr_scheduler(
    ...
    name = cfg.train.scheduler_cfg.name
    if name.lower() == "flat_and_anneal":
        return flat_and_anneal_lr_scheduler(
            ...
            warmup_factor=cfg.train.scheduler_cfg.warmup_factor,
            warmup_iters=cfg.train.scheduler_cfg.warmup_iters,
            warmup_method=cfg.train.scheduler_cfg.warmup_method,  # default "linear"
            anneal_method=cfg.train.scheduler_cfg.anneal_method,
            anneal_point=cfg.train.scheduler_cfg.anneal_point,  # default 0.72
            steps=cfg.train.scheduler_cfg.get("rel_steps", [2 / 3.0, 8 / 9.0]),  # default [2/3., 8/9.], relative decay steps
            target_lr_factor=cfg.train.scheduler_cfg.get("target_lr_factor", 0),
            poly_power=cfg.train.scheduler_cfg.get("poly_power", 1.0),
            step_gamma=cfg.train.scheduler_cfg.gamma,  # default 0.1
    ...
    sched = WarmupParamScheduler(
        sched,
        cfg.train.scheduler_cfg.warmup_factor,
        cfg.train.scheduler_cfg.warmup_iters / total_iters,
        cfg.train.scheduler_cfg.warmup_method,
    )
