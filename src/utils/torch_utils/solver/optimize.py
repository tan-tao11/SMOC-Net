...
from src.utils.utils import msg
...
def _get_optimizer(params, optimizer_cfg, use_hvd=False):
    ...
    if optim_type == "Ranger":
        ...
        from src.utils.torch_utils.solver.ralamb import Ralamb

        optimizer_cls = Ralamb
    elif optim_type == "RAdam":
        from src.utils.torch_utils.solver.radam import RAdam
    ...
    if len(optim_split) > 1 and not use_hvd:
        if optim_split[0].lower() == "lookahead":
            from src.utils.torch_utils.solver.lookahead import Lookahead
